...
void recv_data(void *pvParameters)
{
    int len = 0;	
    char *databuff = (char *)malloc(EXAMPLE_DEFAULT_PKTSIZE * sizeof(char));
	DENG_COMMAND.deng_turn_off = "turnoff";
	DENG_COMMAND.deng_turn_on  = "turnon";
    ...
    while (1) {
        ...
        while (to_recv > 0) {
            ...
            *(databuff+len-1) = '\0';
			...
			if(deng_strcmp(DENG_COMMAND.deng_turn_off, databuff) == 0)
				xEventGroupSetBits(Recv_Event_Handle, LED_TURN_OFF);
			else
				if(deng_strcmp(DENG_COMMAND.deng_turn_on, databuff) == 0)
