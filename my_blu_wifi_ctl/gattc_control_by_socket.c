...
#include "gattc_control_by_socket.h"

uint8_t turnon_light[4]={70,70,70,70};
uint8_t turnoff_light[4]={0,0,0,0};
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_RES_EVT: {
        ...
        if (srvc_id->id.uuid.len == ESP_UUID_LEN_16 && srvc_id->id.uuid.uuid.uuid16 == REMOTE_SERVICE_UUID) {
			...
			ESP_LOGI(GATTC_TAG, "start_handle: %x", p_data->search_res.start_handle);
			ESP_LOGI(GATTC_TAG, "end_handle: %x", p_data->search_res.end_handle);
        }
        break;
    }
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (get_server){
            ...
            esp_gatt_status_t status = esp_ble_gattc_get_attr_count( gattc_if,
                                                                     ...
                                                                     INVALID_HANDLE,
                                                                     &count);
			ESP_LOGE(GATTC_TAG, "the number of attribute %x",count);	
            ...
            if (count > 0){
                ...
                if (!char_elem_result){
                    ...
                    if (count > 0 && (char_elem_result[0].properties & ESP_GATT_CHAR_PROP_BIT_NOTIFY)){
						...
						for(count_number = count;count_number > 0;count_number--)
							{
								ESP_LOGE(GATTC_TAG, "char_handle : %x  propertis : %s uuid : %x",(uint16_t)char_elem_result[count_number].char_handle,(char *)&char_elem_result[count_number].properties,*(uint16_t *)&char_elem_result[count_number].uuid);
							}
...
{   
    ...
    ret = esp_ble_gap_register_callback(esp_gap_cb);//注册一个gap（蓝牙连接的相关操作）回调函数，当有gap回调事件时（回调事件由底层操作）自动跳转到回调函数上进行处理
    if (ret){
    ...
    ret = esp_ble_gattc_register_callback(esp_gattc_cb);//注册一个gatt（蓝牙数据传输的相关操作）回调函数，当有gatt回调事件时（回调事件由底层操作）自动跳转到回调函数上进行处理
    if(ret){
        ESP_LOGE(GATTC_TAG, "%s gattc register failed, error code = %x\n", __func__, ret);
        return;
    }

    ret = esp_ble_gattc_app_register(PROFILE_A_APP_ID);//注册一个profile表a，改表指定相应事件的处理函数
    if (ret){
