...
EventGroupHandle_t Recv_Event_Handle;

MY_COMMAND deng_turn_off_on;
...
void recv_data(void *pvParameters)
{
    int len = 0;	
    char *databuff = (char *)malloc(EXAMPLE_DEFAULT_PKTSIZE * sizeof(char));
	char deng_turn_off_cmp[7] = "turnoff";
    char deng_turn_on_cmp[6]  = "turnon";
	deng_turn_off_on.deng_turn_off = "turnoff";
	deng_turn_off_on.deng_turn_on  = "trunon";
	
    while (1) {
        int to_recv = EXAMPLE_DEFAULT_PKTSIZE;
        while (to_recv > 0) {
            len = recv(connect_socket, databuff, to_recv, 0);
			ESP_LOGI(TAG,"recv return :%d",len);
            *(databuff+len-1) = '\0';
	        ESP_LOGI(TAG,"receive data : %s",databuff);
			if(deng_strcmp(deng_turn_off_on.deng_turn_off, databuff) == 0)
				xEventGroupSetBits(Recv_Event_Handle, LED_TURN_OFF);
			else
				if(deng_strcmp(deng_turn_off_on.deng_turn_on, databuff) == 0)
					xEventGroupSetBits(Recv_Event_Handle, LED_TURN_ON);
				else
					{
					ESP_LOGI(TAG,"error command %d,%d",deng_strcmp(deng_turn_off_on.deng_turn_off, databuff),deng_strcmp(deng_turn_off_on.deng_turn_on, databuff));
					ESP_LOGI(TAG,"compare %d,%d",deng_strcmp(databuff, deng_turn_off_cmp),deng_strcmp(databuff, deng_turn_on_cmp));
					}
